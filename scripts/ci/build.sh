...
#   With no argument (or via make build): builds all 3 platforms.
...
for TARGET in "${TARGETS[@]}"; do
    SUFFIX="${TARGET#bun-}"
    bun build --compile --target="$TARGET" index.ts --outfile "dist/disc-server-${SUFFIX}"
    echo "Built dist/disc-server-${SUFFIX}"
done
